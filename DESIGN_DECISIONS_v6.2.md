...
選取圓以**影像中心**為圓心（非目標星），半徑為影像短邊像素數的一半。以影像中心為圓心可使比較星在空間上對稱分布，各方向大氣梯度的系統誤差互相抵消效果最佳。若以目標星為圓心，當目標星偏離影像中心時，比較星分布不對稱，會引入方向性的大氣梯度殘差。

v6.0 及之前版本程式碼誤以目標星為圓心，v6.1 修正為影像中心。

#### 距離加權

距離 d_i 定義為比較星與**目標星**的角距離（arcsec）。權重公式：
...
其中：
- `d_i`：比較星與目標星的角距離（arcsec）
- `ε = plate_scale / 2`：防止距離趨近零時權重爆炸
- `m_err_i`：星表測光誤差（mag）；無星表誤差時僅用距離項

距離項與誤差項相乘（而非相加），使距離遠且測光誤差大的比較星受雙重壓低。雙重加權未正規化為已知限制（見第五部分）。
...
### 3.9 Ensemble 正規化（Broeg 2005，v6.1 完整實作）

`ensemble_normalize()`：估計每幀的共同大氣漂移 Δ(t)，對目標星星等做修正。

啟用條件：yaml `ensemble_normalize: true` 且通過篩選的比較星數 ≥ `ensemble_min_comp`（預設 3）。預設停用。

#### 演算法（Broeg et al., 2005, eq. 3–4）

**輸入**：各比較星逐幀儀器星等時間序列 `comp_lightcurves`（由 `run_photometry_on_wcs_dir` 在幀迴圈中累積）。

**迭代流程**：

1. 初始權重：以距離＋測光誤差複合權重（`initial_weights`）為種子，不從均等權重開始，利用已有的空間先驗。
2. 計算各比較星偏差：δᵢ(t) = mᵢ(t) − median(mᵢ)，以各星整夜中位數為基準。
3. 計算幀漂移：Δ(t) = Σ wᵢ δᵢ(t) / Σ wᵢ，僅對該幀有效的比較星加總（缺測幀填 NaN，不參與加總）。
4. 扣除漂移後計算每顆比較星殘差 RMS。
5. 更新權重：wᵢ = 1 / RMSᵢ²（Broeg 2005 eq. 4）。
6. 收斂判斷：max |Δ_new(t) − Δ_old(t)| < `ensemble_convergence_tol`（預設 10⁻⁴ mag）則停止；否則回到步驟 2，至多 `ensemble_max_iter`（預設 10）次。

**修正**：m_var_norm(t) = m_var(t) − Δ(t)
...
|----------|------|
| `m_var` | 原始差分星等（保留，不覆蓋） |
| `m_var_norm` | Broeg 正規化後差分星等；ensemble 停用時直通 m_var |
| `delta_ensemble` | Δ(t) 大氣漂移估計值（mag） |

#### 函數回傳值

`run_photometry_on_wcs_dir()` 回傳型別為 `tuple[pd.DataFrame, dict[str, pd.Series]]`：
- 第一個：測光結果 DataFrame（含 m_var_norm）
- 第二個：`comp_lightcurves`，鍵為 `"{ra:.6f}_{dec:.6f}"`，值為 `pd.Series(time → m_inst)`；ensemble 停用時回傳空 dict
...
| 8 | ensemble_normalize() Broeg 迭代收斂性：比較星數極少（= min_comp_stars）時 RMS 更新不穩定 | 預設 min_comp_stars=3；建議實際觀測保留 ≥ 5 顆比較星 |
...
| 低 | 零點回歸函數形式評估 | 線性 vs 二次多項式；待取得 comp_lightcurves 實際資料後，從殘差模式決定 |
...
| 類別 | 變更 | 影響 |
|------|------|------|
| 選取圓圓心修正（§3.5） | 從目標星座標改為影像中心 `(NAXIS1/2, NAXIS2/2)`；`_stars_in_circle` 函數簽名同步更新，移除 `wcs_obj` 和 `target_ra/dec` 參數 | 比較星空間分布對稱，大氣梯度殘差互相抵消效果改善 |
| Broeg ensemble 完整實作（§3.9） | `ensemble_normalize()` 實作 Broeg (2005) 迭代演算法；`run_photometry_on_wcs_dir()` 回傳型別改為 `tuple[pd.DataFrame, dict]`，幀迴圈中累積 `comp_lightcurves` | CSV 新增 `m_var_norm`、`delta_ensemble` 欄位；ensemble 停用時直通 `m_var` |
| yaml 新增欄位 | `photometry` 區塊加入 `ensemble_normalize`、`ensemble_min_comp`、`ensemble_max_iter`、`ensemble_convergence_tol` | 預設全部停用，不影響現有管線行為 |
| 已知限制更新（§5 #8） | ensemble 已完整實作；限制改為「比較星數極少時迭代收斂性」 | — |
| 待實作清單更新（§6） | 移除已完成的兩個高優先項目；新增「零點回歸函數形式評估」低優先項目 | — |
