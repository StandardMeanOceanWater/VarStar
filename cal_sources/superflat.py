# -*- coding: utf-8 -*-
"""
Build a science-frame-derived superflat candidate.
...
_CALIBRATION_IMPORT_ERROR: Optional[Exception] = None
try:
...
except Exception as exc:  # pragma: no cover - import-time cycle guard
    _CALIBRATION_IMPORT_ERROR = exc

    def _raise_calibration_import_error(*args, **kwargs):
        raise ImportError("Calibration helpers are unavailable in this import context") from _CALIBRATION_IMPORT_ERROR

    _dark_rate_paths = _raise_calibration_import_error  # type: ignore[assignment]
    _ensure_master_root = _raise_calibration_import_error  # type: ignore[assignment]
    _list_image_files = _raise_calibration_import_error  # type: ignore[assignment]
    _load_first_existing = _raise_calibration_import_error  # type: ignore[assignment]
    _master_bias_paths = _raise_calibration_import_error  # type: ignore[assignment]
    _master_dark_paths = _raise_calibration_import_error  # type: ignore[assignment]
    _read_frame_data = _raise_calibration_import_error  # type: ignore[assignment]
    _sensor_db_for_session = _raise_calibration_import_error  # type: ignore[assignment]
    compute_dark_rate = _raise_calibration_import_error  # type: ignore[assignment]
    compute_dark_scaled = _raise_calibration_import_error  # type: ignore[assignment]
    load_config = _raise_calibration_import_error  # type: ignore[assignment]
    read_frame_metadata = _raise_calibration_import_error  # type: ignore[assignment]
...
def _infer_from_raw_dir(raw_dir: Path) -> Tuple[str, str]:
    ...
    try:
        ...
        raise ValueError(f"raw directory is not under data/: {raw_dir}") from exc

    if idx + 2 >= len(parts):
        raise ValueError(f"cannot infer date/group from raw directory: {raw_dir}")
    date_part = parts[idx + 1]
    group_part = parts[idx + 2]
    date_digits = date_part.replace("-", "")
    if not re.fullmatch(r"\d{8}", date_digits):
        raise ValueError(f"invalid date token: {date_part}")
    ...
    for session in cfg.get("obs_sessions", []):
        ...
        for target_name in targets:
            if target_cfg.get(target_name, {}).get("group") == group_name:
                return dict(session)
    raise ValueError(f"no session matches date={session_date}, group={group_name}")
    ...
    for path in tqdm(files, desc="scan frames", unit="file"):
        try:
            ...
            if light_data.shape != ref_shape:
                print(f"  [WARN] shape mismatch, skipped: {path.name} ({light_data.shape} != {ref_shape})")
                continue
            ...
            if sat_frac > sat_threshold:
                print(f"  [WARN] saturation fraction too high, skipped: {path.name} sat_frac={sat_frac:.4f}")
                continue
            scans.append(FrameScan(path=path, norm=norm, sat_frac=sat_frac, exposure_s=meta.get("exposure_s")))
        except Exception as exc:
            print(f"  [WARN] prescan failed, skipped: {path.name} ({exc})")

    if ref_shape is None:
        raise ValueError("no readable light frame found")
...
def _superflat_stack(
    ...
    for chunk_index, start in enumerate(range(0, len(scans), chunk_size), start=1):
        ...
        for item in tqdm(batch, desc=f"stack chunk {chunk_index}", unit="file", leave=False):
            try:
                ...
                if light_data.shape != ref_shape:
                    print(f"  [WARN] shape mismatch, skipped: {item.path.name}")
                    continue
                calibrated = light_data.astype(np.float64)
                if master_bias is not None:
                    calibrated -= master_bias.astype(np.float64)
                ...
                print(f"  [WARN] stacking failed, skipped: {item.path.name} ({exc})")
        if stack:
    ...
    if not chunk_medians:
        raise ValueError("no usable light frame remained for superflat")
...
def _run_one_format(
    ...
    if len(kept) < 10:
        raise ValueError(f"too few usable frames: {len(kept)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a superflat from a raw directory")
    parser.add_argument("--raw-dir", type=Path, required=True, help="Example: data/2025-12-20/Ori/raw")
    ...
    parser.add_argument("--output", type=Path, help="Single-format output path; multi-format mode appends _fits/_cr2")
    parser.add_argument("--chunk-size", type=int, default=10)
    parser.add_argument("--limit", type=int, help="Use only the first N frames per format")
    ...
    parser.add_argument("--filter-sigma", type=float, default=3.0, help="MAD-based norm filter threshold")
    parser.add_argument("--sat-threshold", type=float, default=0.02, help="Upper bound for bright-tail pixel fraction")
    ...
    if not grouped:
        raise ValueError(f"no processable {args.format} files found in: {raw_dir}")
...
def candidate_superflat_paths(
    ...
    candidates: List[Path] = []
    for group in groups:
        safe_group = _ascii_safe(str(group), "group")
        candidate = master_root / f"superflat_{session_date}_{safe_group}_{frame_type}.fits"
        if candidate not in candidates:
            candidates.append(candidate)
    if master_root.exists():
        pattern = f"superflat_{session_date}_*_{frame_type}.fits"
        for path in sorted(master_root.glob(pattern), key=lambda item: item.name):
            if path not in candidates:
                candidates.append(path)
    return candidates


def validate_superflat_array(data: np.ndarray, min_finite_ratio: float = 0.80) -> Tuple[bool, str]:
    finite_ratio = float(np.mean(np.isfinite(data)))
    median_value = float(np.nanmedian(data))
    if finite_ratio < min_finite_ratio:
        return False, f"finite_ratio={finite_ratio:.3f} < {min_finite_ratio:.3f}"
    if not math.isfinite(median_value) or median_value <= 0.0:
        return False, f"median={median_value:.6f} invalid"
    return True, "ok"
