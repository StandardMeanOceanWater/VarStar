# -*- coding: utf-8 -*-
"""
split.py  --  Bayer split module.

Project: VarStar photometry pipeline.
Description:
    Read WCS FITS frames and split the Bayer mosaic into R / G1 / G2 / B
    channels by stride-2 slicing. This is not demosaic interpolation.

    FITS headers are inherited, and WCS keywords are adjusted for the
    stride-2 sub-sampling (CRPIX and CD/CDELT).

WCS propagation rule
--------------------
...
def detect_bayer_pattern(
    ...
    """
    Detect the Bayer pattern from FITS header or YAML default.

    Priority:
    1. FITS BAYERPAT
    2. FITS COLORTYP
    3. YAML camera default
    4. RGGB fallback
    """
...
def _propagate_wcs(
    ...
    """
    Convert full-frame WCS to the stride-2 channel WCS.
    """
    h = header_orig.copy()
...
def _build_channel_header(
    ...
    """
    Build a channel FITS header after Bayer splitting.
    """
...
def run_split(
    ...
    *,
    ...
    """
    Main Bayer split pipeline entry point.

    Reads `observation_config.yaml`, processes each session's group-level WCS
    FITS files, and writes channel FITS files under
    `splits/{R,G1,G2,B}` or the requested split subdirectory.
    """
    ...
    print("\n" + "=" * 60)
    print(f"  變星測光管線 — Bayer split 模組  split.py{_mode_tag}")
    print("=" * 60)
    ...
    for session in sessions:
        ...
        for _group, group_target_names in group_targets.items():
            ...
            for fits_path in tqdm(wcs_files, desc=f"{_group} split 進度"):
                try:
                    base_name = fits_path.stem
                    expected_outputs = [
                        split_base / ch / f"{base_name}_{ch}.fits"
                        for ch in ("R", "G1", "G2", "B")
                    ]
            ...
            print(f"\n[完成] {session_label}/{date}：成功 {success} 幀，已跳過 {skipped} 幀，失敗 {failed} 幀")
            print(f"       輸出目錄：{split_base}/{{R,G1,G2,B}}/")
            if success + skipped == 0 and failed > 0:
                raise RuntimeError(f"split produced no usable frames for {session_label}/{date}")

    print("\n" + "=" * 20)
    print("所有 Session split 完成。")
    print("G2 通道僅供品質驗證（G1 vs G2 residual rms = PRNU 估計值）。")
    print("下一步：Photometry.ipynb")
    print("=" * 20)


# Legacy import compatibility. New code should call run_split.
run_debayer = run_split
