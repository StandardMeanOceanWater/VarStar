...
**版號：v1.03 | 最後更新：2026-03-14 | 修訂規則見 PIPELINE_STATUS.md 開頭**
...
├── pipeline\                        ← 工作目錄（本倉庫）
│   ├── CLAUDE.md                    ← 本檔
│   ├── PIPELINE_STATUS.md           ← 模組狀態快照（每次對話後更新）
│   ├── DESIGN_DECISIONS_v6.md       ← 唯一學理規格（v6.1）
...
└── data\
    ├── shared_calibration\
    │   ├── {date}\  {bias/, dark/, flat/, masters/}
    │   └── {date}\  dark/ 可有溫度子目錄 e.g. {3.7C/, 5.3C/}
    └── targets\
        └── {TARGET}\
